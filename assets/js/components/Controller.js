...
export default class Controller extends Component {
  ...
  render() {
    ...
    const blockCellInAlignment = (
      cpuMark,
      oppMark,
      cpuCells,
      noOppMarkInLine = true
    ) => {
      let cellsCheck = [];
      let falseCount = 0;
      ...
      const checkCellInAlignment = (cellIdx, callback) => {
        ...
        cellsCheck.push(cellText === cpuMark || cellText === oppMark);
          ...
          if (!game.player2IsHuman) {
            ...
            if (game.player1 === "O" && game.nextTurn === "X") {
              if (game.movesCount === 2) {
                blockCellInAlignment("X", "O", game.xCells, true);
              }
              if (game.movesCount >= 4) {
                blockLastCell("X", game.xCells) ||
                  blockLastCell("O", game.oCells) ||
                  blockCellInAlignment("X", "O", game.xCells) ||
                  blockCellInAlignment("X", "O", game.oCells) ||
                  blockCellInAlignment("X", "O", game.oCells, false) ||
                  blockCellInAlignment("X", "O", game.xCells, false) ||
              ...
              if (game.movesCount === 1) {
                ...
                blockLastCell("O", game.oCells) ||
                  blockLastCell("X", game.xCells) ||
                  blockCellInAlignment("O", "X", game.oCells) ||
                  blockCellInAlignment("O", "X", game.xCells) ||
                  blockCellInAlignment("O", "X", game.xCells, false) ||
                  blockCellInAlignment("O", "X", game.oCells, false) ||
                  blockCellInAlignment(
                    "O",
                    "X",
                    game.oCells.slice(0, game.oCells.length - 1),
